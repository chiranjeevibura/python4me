...
def write_data_to_mongodb(df):
    ...
    df_transformed = df.withColumn("_id", F.expr("UUID()"))

    df_transformed = df.withColumn("employee_exit", lit("isCurrent"))

    df_transformed = df.withColumn("gender", col("gender"))

    df_transformed.write.format().mode("append").option("uri", mongo_uri).option("database", mongo_db).option("collection", mongo_coll).save()

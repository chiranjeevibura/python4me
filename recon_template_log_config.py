...
for batch in pending_batches:
    ...
    logger.info(f"Executing SQL Query for Source Data: {sql_query}")
    ...
    logger.info(f"Source Data for BATCHID={batch_id}: {source_df.show(truncate=False)}")
    ...
    logger.info(f"Target Data for BATCHID={batch_id}: {target_df.show(truncate=False)}")

    # Step 4: Check if Source or Target DataFrame is Empty
    source_count = source_df.count()
    target_count = target_df.count()

    if source_count == 0 or target_count == 0:
        reconcile_check = "NOT MATCHED"
        retrieval_validation_result = "FAILURE"

        logger.info(f"Source or Target data is empty for BATCHID={batch_id}. Marking as NOT MATCHED.")
        
        update_query = f"""
            UPDATE SparkReconBatchTable 
            SET RECONCILE_CHECK = '{reconcile_check}', 
                RECONCILE_SRC_CNT = {source_count}, 
                RECONCILE_TGT_CNT = {target_count},
                RETRIEVAL_VALIDATION = '{retrieval_validation_result}'
            WHERE BATCHID = '{batch_id}'
        """
        cursor.execute(update_query)
        conn.commit()

        # Skip rest of validation for this batch
        continue
    ...
    logger.info(f"Data Comparison for BATCHID={batch_id}:\n{comparison_df.show(truncate=False)}")
    logger.info(f"Mismatched Data for BATCHID={batch_id}:\n{mismatch_df.show(truncate=False)}")
    ...
    def validate_storage_paths(batch_df):
        ...
        for row in batch_df.collect():
            ...
            save_path = f"{save_directory}/downloaded_object_{object_guid}.dat"
            status = hcp_search_and_download_object(hcp_url, object_guid, auth_code, save_path)
            logger.info(f"GUID Retrieval for {object_guid}: {status}")
            results.append(status)
        return ",".join(results)
    
    guid_df = target_df.select("documentOsGuid", "storage.documentStoreUrl").limit(100)
    
    logger.info(f"GUIDs selected for validation for BATCHID={batch_id}: {guid_df.show(truncate=False)}")
    ...
    logger.info(f"Completed reconciliation for BATCHID={batch_id}")
